...
public class YouqinglianjieController extends BaseController
{
    ...
    public String list()
    {
        ...
        List<Youqinglianjie> list = service.selectPageExample(example , page , 12);   // 获取当前页的行数
        // 将列表写给界面使用
        assign("list" , list);
        assign("orderby" , order);  // 把当前排序结果写进前台
        assign("sort" , sort);      // 把当前排序结果写进前台
        assign("where" , where);    // 把当前条件写给前台
        return "youqinglianjie_list";   // 使用视图文件：WebRoot\youqinglianjie_list.jsp
    }
    ...
    */
    ...
    public String delete()
    {
        ...
        HashMap delMap = Query.make("youqinglianjie").find(id);
